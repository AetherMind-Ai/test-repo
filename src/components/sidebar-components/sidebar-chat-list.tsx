...
const SidebarChatList = ({ sidebarList }: any) => {
  ...
  const { setTopLoader, setToast } = MindBotZustand();
        ...
        return (
          <div className="mt-5">
            <p>
              ...
              activity like prompts, responses, and feedback from your MindBot Ai
              Apps Activity.
            </p>
          ...
          sidebarList.message.map((item: any) => (
            <li key={item.chatID}>
              <DevButton
                ...
                className={clsx(
                  "text-sm overflow-hidden group !w-full justify-between relative !pr-2 gap-4 rounded-xl", // added rounded-xl here and removed rounded = "full"
                  item.chatID === chat
                  && "!bg-accentBlue/50"
                )}
                // Added hover state styles directly within clsx
                style={{
                   borderRadius: '35px',
                }}
              >
                  ...
                  <div className="overflow-hidden py-2 min-w-44">
                    ...
                    >
                       <MdDeleteOutline
                          className="text-xl"
                          style={{
                              color: settings === item.chatID ? 'red' : 'inherit', // Conditionally apply the red color
                          }}
                      />
                      Delete
                    </DevButton>
